...
void SlidingWindow(char* readstring, ACKmngr* ACKsPointer) {
	...
	DEBUGMESSAGE(3, BLU"\n----------------------Sending Packet:["RESET" %d "BLU"]   seq:["RESET" %d "BLU"]   messageTracker:["RESET" %d "BLU"]"RESET,
	...
	DEBUGMESSAGE(3, YEL"Message: ["RESET" %d "YEL"] Sent     "MAG"ACKs.Missing:["RESET" %d "MAG"]"RESET,
...
int main(int argc, char* argv[]) {
	...
	switch (command) {
		...
		connectetionEstablished = 1;
		usleep(5000);
		break;
	    case 2:
		if (connectetionEstablished != 1) {
		    system("clear");
		    printf(RED"Connection not established!\n"RESET);
		} else {
		    system("clear"); // Clean up the console
		    ...
		    //----------------------------------------------------------------
		    usleep(5000);
		    printf(YEL"Reading message from file...\n"RESET);
